    ...
    const fetchMaxChunks = async () => {
      ...
      try {
        const response = await fetch(
          //`http://localhost:8000/api/text_chunks/${prefix}_${filename}?chunk_num=${chunkNum}&chunk_size=${chunkSize}`,
          `/api/text_chunks/${prefix}_${filename}?chunk_num=${chunkNum}&chunk_size=${chunkSize}`,
          {
            method: 'GET',
            mode: 'cors',
            headers: {
